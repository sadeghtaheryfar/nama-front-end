...
const FormSabt = ({ id, data }) => {
    ...
    const [allCoachesForSelection, setAllCoachesForSelection] = useState({
        data: [],
    }); // All coaches globally available for selection
    const [loadingCoachesForSelection, setLoadingCoachesForSelection] =
        useState(false); // Loading for all coaches
        ...
        const fetching = async () => {
            try {
                const response = await axios.get(
                    `/api/show-item-dashboard?item_id=${currentItemId}&role=mosque_head_coach`,
                );
                if (response.data) {
    ...
    useEffect(() => {
        const fetchAllRings = async () => {
            ...
            try {
                ...
                const response = await axios.get(
                    `/api/loop/index?item_id=${itemId}&role=mosque_head_coach`,
                );
                if (response.data) {
    ...
    useEffect(() => {
        ...
        const fetchAllCoachesForSelection = async () => {
            ...
            try {
                ...
                const response = await axios.get(
                    `/api/loop/index?item_id=${itemId}&role=mosque_head_coach&type=${type}`,
                ); // Added a hypothetical 'type=coaches'
                if (response.data && response.data.data) {
                    // Filter out ring data and keep only coach data if mixed
                    const coaches = response.data.data.filter(
                        (item) => item.name && item.national_code,
                    ); // Simple heuristic
    ...
    useEffect(() => {
        ...
        const fetchMembersForSelectedRing = async () => {
            if (selectedRingId && itemId) {
                ...
                try {
                    const response = await axios.get(
                        `/api/loop/show?item_id=${itemId}&id=${selectedRingId}&type=${type}`,
                    );
                    if (
                        response.data &&
                        response.data.data &&
                        ...
                        const initialMembers = response.data.data.members.map(
                            (member) => ({
                                id: member.id,
                                name: member.name,
                            }),
    ...
    useEffect(() => {
        return () => {
            ...
            connectionLetters.forEach((file) =>
        ...
        setFiles((prevFiles) => {
            ...
            if (combinedFiles.length > 10) {
                ...
                setErrors((prevErrors) => ({
                    ...prevErrors,
                    [fieldName]: "حداکثر 10 فایل مجاز است",
                }));
            ...
            if (setStatus)
                setStatus(
                    newFiles.length > 0
                        ? "فایل(های) مورد نظر انتخاب شد"
                        : "مقدار فایل وجود ندارد",
                );
            setErrors((prevErrors) => ({
                ...prevErrors,
                [fieldName]: hasInvalidType
    ...
    const removeFile = (setFiles, idToRemove, fieldName) => {
        setFiles((prevFiles) => {
            ...
            if (
                fieldName === "imamLetter" &&
                isImamLetterRequired &&
                updatedFiles.length === 0
            ) {
                setErrors((prevErrors) => ({
                    ...prevErrors,
                    [fieldName]: `فایل نامه الزامی است`,
                }));
            } else if (
                ...
                setErrors((prevErrors) => ({
                    ...prevErrors,
                    [fieldName]: "پیوست های بیشتر الزامی است",
                }));
            } else if (
                fieldName === "connectionLetter" &&
                isAreaLetterRequired &&
                updatedFiles.length === 0
            ) {
                setErrors((prevErrors) => ({
                    ...prevErrors,
                    [fieldName]: "فایل نامه رابط منطقه الزامی است",
                }));
            } else {
        ...
        if (data?.type !== "university") {
            ...
            if (connectionLetters.length > 0) {
                formDataToSend.append(
                    ...
                    connectionLetters[0].file,
                );
                for (let i = 1; i < connectionLetters.length; i++) {
                    formDataToSend.append(
                        `other_area_interface_letter[${i - 1}]`,
                        connectionLetters[i].file,
                    );
                }
            }
        }
        ...
        try {
            ...
            if (submitForm.data) {
                localStorage.setItem(
                    "submittedForm",
                    JSON.stringify(submitForm.data),
                );
                router.push(
    ...
    const RequiredStar = () => (
        <span className="text-red-500 mr-1" style={{ fontFamily: "none" }}>
            *
        </span>
    );
    ...
    const combinedAndFilteredCoaches = Array.from(
        new Map(
            [...AllRingMembers, ...selectedRingMembers].map((item) => [
                item["id"],
                item,
            ]),
        ).values(),
    ...
    return (
        <div className="w-full bg-white rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-[auto,auto] md:gap-x-2 xl:grid-cols-3 xl:gap-x-6 2xl:gap-x-8">
                ...
                {data?.show_ring_member && (
                    <div className="mb-4">
                        ...
                        <div
                            className={`grid grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 border rounded-lg ${getBorderStyle("selectedRingId")}`}
                        >
                            {loadingRings ? (
                                <div className="col-span-2 text-center text-gray-500">
                                    در حال بارگذاری حلقه‌ها...
                                </div>
                            ) : availableRings.data &&
                              availableRings.data.length > 0 ? (
                      ...
                      ${
                          selectedRingId === ring.id
                              ...
                              : "bg-gray-100 text-gray-800 border-gray-300 hover:bg-gray-200"
                                        ...
                                        onClick={() =>
                                            handleRingSelection(ring.id)
                                        }
                                    >
                                        {ring.title || `حلقه ${ring.id}`}{" "}
                                        {/* Display ring name or ID */}
                                    </div>
                                ))
                            ) : (
                                <div className="col-span-2 text-center text-gray-500">
                ...
                {data?.show_ring_member &&
                    selectedRingId && ( // Only show if a ring is selected
                                ...
                                <div className="absolute z-10 w-full bg-white border border-gray-300 rounded-lg mt-1 shadow-lg max-h-60 overflow-y-auto">
                                    ...
                                    {loadingCoachesForSelection ? ( // Use loading state for all coaches
                                        <div className="p-4 text-center text-gray-500">
                                            در حال بارگذاری مربیان...
                                        </div>
                                    ) : combinedAndFilteredCoaches.length >
                                      0 ? (
                                        combinedAndFilteredCoaches.map(
                                            (member) => (
                                                <div
                                                    key={member.id}
                                                ...
                                                >
                                                    <input
                                                        type="checkbox"
                                                        checked={selectedRingMembers.some(
                                                            (m) =>
                                                                m.id ===
                                                                member.id,
                                                        )}
            ...
            <div className="flex items-start mb-7 mt-7">
                ...
                >
                    تمامی اطلاعات فوق را بررسی کرده ام و صحت آن را تایید می کنم
                    و در صورت عدم تطبیق مسئولیت آن را می پذیرم. <RequiredStar />
                </label>
