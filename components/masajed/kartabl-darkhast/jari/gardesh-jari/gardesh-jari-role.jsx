...
const GardeshJariRole = ({data}) => {
  ...
  const getStepText = (stepName, associatedRole) => {
    ...
    if (status === 'current_in_progress_responsible') {
      return `${stepTitles[stepName]} (جاری)`;
      ...
      return `${stepTitles[stepName]} (رد شده)`;
    } else if (status === 'current_action_needed') {
      return `${stepTitles[stepName]} (نیازمند اصلاح)`;
    }
      ...
      <div className="hidden lg:flex items-end justify-between">
        {stepsConfig.map((stepItem, index, array) => (
          <div key={stepItem.name} className={`w-full flex flex-col items-start relative`}>
            ...
            <span className={`text-sm ${getStepColorClass(stepItem.name, stepItem.associatedRole)} mt-4 xl:mt-6 xl:text-lg 2xl:mt-8 2xl:text-[22px]  min-w-fit ${index > 0 ? 'lg:mr-[-28px] 2xl:mr-[-40px]' : ''}`}>
