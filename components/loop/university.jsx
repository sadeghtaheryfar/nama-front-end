        ...
        const fetchData = async () => {
            ...
            try {
                const data = await axios.get(
                    `/api/loop/index?item_id=${itemId}&role=${roleEn}&page=${currentPage}&per_page=${itemsPerPage}`
                );
                if (data.data) {
                    setData(data.data);
                    ...
                    if (data.data.meta && data.data.meta.total) {
                        setTotalPages(
                            Math.ceil(data.data.meta.total / itemsPerPage)
                        );
                    } else {
                        setTotalPages(
                            Math.ceil(data.data.data.length / itemsPerPage) || 1
                        );
                    }
                }
            } catch (error) {
    ...
    const exportfile = () => {
        ...
        fetch(url, {
            ...
            .then((blob) => {
                if (blob.size === 0) {
                    toast.error(
                        "فایل دریافت شده خالی است. لطفاً با پشتیبانی تماس بگیرید."
                    );
                    return;
                }
                ...
                link.href = blobUrl;
                link.download = `exported_data_${new Date()
                    .toISOString()
                    .slice(0, 10)}.xlsx`;
            ...
            .catch((error) => {
                ...
                toast.error(
                    `خطا در دانلود فایل: ${error.message || "مشکل ناشناخته"}.`
                );
            })
            .finally(() => {
            ...
            {data && (
                <section className="mt-[2rem]">
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="text-[14px] lg:text-[20px]">
                                دانشگاه
                            </h3>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="hover:scale-[1.04] active:scale-[1] transition-[0.9s] py-[0.5rem] text-[#c33232] px-[0.75rem] lg:px-[1rem] rounded-[0.5rem] shadow-lg flex justify-center items-center gap-2 text-[10px] lg:text-[14px]"
                            >
                            ...
                            <Link
                                href={`/loop/add?item_id=${itemId}`}
                                className="hover:scale-[1.04] active:scale-[1] transition-[0.9s] bg-[#c33232] text-white py-[0.5rem] px-[0.75rem] lg:px-[1rem] rounded-[0.5rem] shadow-lg flex justify-center items-center gap-2 text-[10px] lg:text-[14px]"
                            >
                                <span className="hidden lg:block">
                                    برای افزودن حلقه کلیک کنید
                                </span>
                    ...
                    <div className="mt-[2rem] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        {data &&
                            ...
                            data?.data?.map((item) => (
                                <div
                                    key={item?.id}
                                    className="w-full border border-[#DFDFDF] rounded-[1rem] p-[1.5rem] group hover:border-[#c33232] transition-all duration-200"
                                >
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div>
                                            <Link
                                                href={`/loop/update?id=${item?.id}&item_id=${itemId}`}
                                                className="hover:scale-[1.08] active:scale-[1] transition-[0.9s] flex justify-center items-center gap-2 text-[#c33232] font-semibold text-[14px]"
                                            >
                                    ...
                                    <div className="text-[14px] text-[#818595] mt-[0.5rem]">
                                        ...
                                        <p>
                                            تعداد اعضای حلقه:{" "}
                                            {item?.members_count} نفر
                                        </p>
                                    </div>

                                    <div className="w-full">
                                        <Link
                                            href={`/loop/show?id=${item?.id}&item_id=${itemId}`}
                                            className="hover:scale-[1.04] active:scale-[1] transition-[0.9s] text-[#c33232] border border-[#c33232] group-hover:text-white group-hover:bg-[#c33232] transition-all duration-200 rounded-[0.5rem] w-full flex justify-center items-center p-[0.5rem] mt-[0.5rem]"
                                        >
                                            مشاهده بیشتر
                                        </Link>
                                    </div>
