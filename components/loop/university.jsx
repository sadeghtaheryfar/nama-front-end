            ...
            {data && (
                <section className="mt-[2rem]">
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="text-[14px] lg:text-[20px]">دانشگاه</h3>
                        </div>

                        <div className="flex gap-2">
                            <button onClick={(e) => exportfile()} className="hover:scale-[1.04] active:scale-[1] transition-[0.9s] py-[0.5rem] text-[#c33232] px-[0.75rem] lg:px-[1rem] rounded-[0.5rem] shadow-lg flex justify-center items-center gap-2 text-[10px] lg:text-[14px]">
                                <span className="hidden lg:block">{loadingExport ? 'صبر کنید ...' : 'خروجی اکسل دانشگاه'}</span>
                    ...
                    <div className="mt-[2rem] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        {(data && data?.data?.length > 0) && 
                            data?.data?.map((item) => (
                                <div key={item?.id} className="w-full border border-[#DFDFDF] rounded-[1rem] p-[1.5rem] group hover:border-[#c33232] transition-all duration-200">
                                    <div className="flex justify-between items-start">
                                        <div className="w-[50%]">
                                            <img className="w-[50%]" src="/Images/Growth.png" alt="" />
                                        </div>

                                        <div>
                                            <Link href={`/loop/update?id=${item?.id}&item_id=${itemId}`} className="hover:scale-[1.08] active:scale-[1] transition-[0.9s] flex justify-center items-center gap-2 text-[#c33232] font-semibold text-[14px]">
                                                <span>ویرایش حلقه</span>

                                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M11 2H9C4 2 2 4 2 9V15C2 20 4 22 9 22H15C20 22 22 20 22 15V13" stroke="#c33232" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                                <path d="M16.04 3.01976L8.16 10.8998C7.86 11.1998 7.56 11.7898 7.5 12.2198L7.07 15.2298C6.91 16.3198 7.68 17.0798 8.77 16.9298L11.78 16.4998C12.2 16.4398 12.79 16.1398 13.1 15.8398L20.98 7.95976C22.34 6.59976 22.98 5.01976 20.98 3.01976C18.98 1.01976 17.4 1.65976 16.04 3.01976Z" stroke="#c33232" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                                                <path d="M14.91 4.1499C15.58 6.5399 17.45 8.4099 19.85 9.0899" stroke="#c33232" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                                                </svg>
                                            </Link>
                                        </div>
                                    </div>

                                    <div className="text-[20px] font-semibold mt-[1rem] group-hover:text-[#c33232] transition-all duration-200">
                                        <p>دانشگاه: {item?.title} </p>
                                    </div>
                                    ...
                                    <div className="w-full">
                                        <Link href={`/loop/show?id=${item?.id}&item_id=${itemId}`} className="hover:scale-[1.04] active:scale-[1] transition-[0.9s] text-[#c33232] border border-[#c33232] group-hover:text-white group-hover:bg-[#c33232] transition-all duration-200 rounded-[0.5rem] w-full flex justify-center items-center p-[0.5rem] mt-[0.5rem]">مشاهده بیشتر</Link>
                                    </div>
                                </div>
                            ))
                        }
