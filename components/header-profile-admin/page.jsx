...
const Header = ({ bgBox, bgRole }) => {
    ...
    useEffect(() => {
        ...
        if (
            !itemIdFromUrl ||
            !validItemIds.includes(itemIdFromUrl) ||
            !roleFromUrl ||
            !validRoles.includes(roleFromUrl)
        ) {
            router.replace("/");
        }
    }, [itemIdFromUrl, roleFromUrl, router, roleOptions, user]);
    ...
    const portalMenu =
        showRoleMenu && typeof window !== "undefined"
            ? createPortal(
                  <div
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-1 leading-5">
                <span
                    className="text-xs lg:text-lg font-medium text-right text-ellipsis overflow-hidden max-w-[5rem] lg:max-w-none whitespace-nowrap"
                    dir="rtl"
                >
                    سلام {user?.data?.name || " "}!
                </span>
