...
export default function Role() {
    ...
    const { banners, headerData } = useSelector((state) => state.dashboards);
    ...
    useEffect(() => {
        ...
        if (banners.data && banners.itemId === item_id) {
        ...
        fetching();
    }, [item_id, banners.data, banners.itemId, dispatch]);

    useEffect(() => {
        if (!item_id) return;

        if (headerData && String(headerData.data?.id) === String(item_id)) {
        ...
        const fetching = async () => {
            try {
                ...
                console.log("خطا در دریافت هدر:", error);
            } finally {
                setLoadingHeader(false);
            }
        };
        fetching();
    }, [item_id, headerData, dispatch]);
                    ...
                    <div className="bg-white absolute top-[150px] md:top-[160px] inset-x-6 md:inset-x-11 rounded-[28px] p-3 md:p-6 scroll-kon">
                        ...
                        <div className="mt-7 grid grid-cols-1 lg:grid-cols-2 md:mt-9 gap-6 md:gap-7">
                            <div className="flex flex-col justify-end gap-5 border rounded-xl p-6 group hover:border-[#345894] transition-all duration-200">
                                ...
                                <h2 className="text-base font-extrabold group-hover:text-[#345894]">
                                    کارتابل درخواست ها
                                </h2>
                                ...
                                <Link
                                    href={`/role/kartabl?role=${role}&item_id=${item_id}`}
                                    className="mt-auto"
                                >
                                    <button className="w-full h-12 text-[#345894] text-base font-medium rounded-[10px] border border-[#345894] group-hover:bg-[#345894] group-hover:text-white hover:scale-[1.03] active:scale-[1] transition-[0.9s]">
                                        مشاهده بیشتر
                                    </button>
                                </Link>
                            </div>
                            ...
                            <div className="flex flex-col justify-end gap-5 border rounded-xl p-6 group hover:border-[#345894] transition-all duration-200">
                                ...
                                <h2 className="text-base font-bold group-hover:text-[#345894]">
                                    کارتابل گزارش ها
                                </h2>
                                <p className="text-xs font-medium text-slate-400 leading-6">
                                    لورم ایپسوم متن ساختگی با تولید سادگی
                                ...
                                <Link
                                    href={`/role/kartabl-gozaresh?role=${role}&item_id=${item_id}`}
                                    className="mt-auto"
                                >
                                    <button className="w-full h-12 text-[#345894] text-base font-medium rounded-[10px] border border-[#345894] group-hover:bg-[#345894] group-hover:text-white hover:scale-[1.03] active:scale-[1] transition-[0.9s]">
                                        مشاهده بیشتر
